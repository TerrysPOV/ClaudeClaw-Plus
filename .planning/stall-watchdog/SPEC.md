...
3. **Distinguishes stall from legit-long tool by the tool's own bound**, not silence (silence is identical for both at the transcript level). Ceilings are keyed on the `tool_use` name:
   - fast tools (`Read`, `Edit`, `Grep`, `Glob`, `Write`, `LS`) → short ceiling (warn 60s / kill 120s).
   - `Bash` → generous (warn 300s / kill 900s = its 10-min max + grace).
   - MCP / unknown tools → configurable default (warn 300s / kill 900s).
...
**Modify:**
- `src/config.ts` — add a `stallWatchdog` settings block + `parseStallWatchdogConfig` (mirror `parseWatchdogConfig` in `src/watchdog.ts:136`), wire into `parseSettings` and `DEFAULT_SETTINGS` (near the existing `watchdog:` at `~:218`/`~:1010`). Shape: `{ enabled, sweepIntervalMs, ceilings: { default:{warnSeconds,killSeconds}, perTool?: Record<string,{warnSeconds,killSeconds}> }, action: "restart"|"warn", autoDiscovery: { enabled, cpuProbeMs } }`.
