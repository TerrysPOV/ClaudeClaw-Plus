...
export class BusCoreImpl implements BusCore {
  ...
  private readonly inFlightProof = new Map<
    string,
    Map<string, { proven: boolean; token: number }>
  >();
  ...
  private readonly compactionHeld = new Map<
    string,
    Map<string, { wrapped: string; timer: ReturnType<typeof setTimeout>; generation?: number }>
  >();
        ...
        if (agentId) {
          ...
          if (inFlight)
            for (const [k, wrapped] of inFlight)
              if (proofs?.get(k)?.proven === false) carryOver.add(wrapped);
          this.inFlightProof.delete(agentId);
          this.inFlightWrapped.delete(agentId);
          this.inFlightEpoch.set(agentId, (this.inFlightEpoch.get(agentId) ?? 0) + 1);
    ...
    void this.streamPromptHandler(agent_id, wrapped)
      .then((outcome) => {
        ...
        if (
          ...
          owns() &&
          this.inFlightProof.get(agent_id)?.get(key)?.proven === false &&
  ...
  private holdThroughCompaction(agent_id: string, wrapped: string): void {
    ...
    held.set(key, {
      ...
      generation: this.agentTailerGeneration.get(agent_id),
      timer: setTimeout(() => {
        this.onError(
          new Error(
  ...
  private releaseCompactionHold(
    agent_id: string,
    only?: { key?: string; notGeneration?: number; generation?: number },
  ): void {
    ...
    if (e.agent_id) {
      ...
      // never will be), so the held prompt can be verified.
        ...
        this.releaseCompactionHold(
          e.agent_id,
          typeof gen === "number" ? { generation: gen } : undefined,
        );
      }
