...
async function handleMessage(message: TelegramMessage): Promise<void> {
  ...
  const hasMedia = Boolean(
    message.video || message.animation || message.sticker || message.video_note,
  );
  ...
  if (!isAllowed(userId, config.allowedUserIds)) {
    if (isPrivate) {
      ...
      debugLog(
        `Skip group message chat=${chatId} from=${userId ?? "unknown"} reason=unauthorized_user`,
      );
    }
    return;
  }
  ...
  try {
    ...
    if (voiceTranscript) {
      promptParts.push(
        `Voice transcript: ${wrapUntrusted("voice-transcript", voiceTranscript, 2000)}`,
      );
    } else if (voicePath) {
      const { delegateTool } = getSettings().stt;
      if (delegateTool) {
    ...
    if (documentInfo) {
      ...
      promptParts.push(
        `Original filename: ${wrapUntrusted("attachment-filename", documentInfo.originalName)}`,
      );
    ...
    if (mediaInfo) {
      ...
      promptParts.push(
        `Original filename: ${wrapUntrusted("attachment-filename", mediaInfo.originalName)}`,
      );
...
async function poll(generation: number): Promise<void> {
  ...
  console.log(
    `  Allowed users: ${config.allowedUserIds.length === 0 ? "none (deny all)" : config.allowedUserIds.join(", ")}`,
  );
